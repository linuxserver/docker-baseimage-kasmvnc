      ...
      steps {
        sh '''#! /bin/bash
              ...
              if [ "${NEW_PACKAGE_TAG}" != "${PACKAGE_TAG}" ]; then
                ...
                git --git-dir ${TEMPDIR}/${LS_REPO}/.git checkout -f alpine319
          ...
          retry_backoff(5,5) {
            sh '''#! /bin/bash
                  ...
                  for PUSHIMAGE in "${GITHUBIMAGE}" "${GITLABIMAGE}" "${QUAYIMAGE}" "${IMAGE}"; do
                    ...
                    docker buildx imagetools create -t ${PUSHIMAGE}:alpine319 ghcr.io/linuxserver/jenkins-builder:empty || true
        ...
        withCredentials([
          ...
          retry_backoff(5,5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    docker buildx imagetools create -t ${MANIFESTIMAGE}:alpine319 -t ${MANIFESTIMAGE}:amd64-alpine319 -t ${MANIFESTIMAGE}:arm64v8-alpine319 ghcr.io/linuxserver/jenkins-builder:empty || true
