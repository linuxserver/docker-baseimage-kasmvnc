    ...
    stage("Set ENV PR build"){
      ...
      steps {
        script{
          ...
          if (env.MULTIARCH == 'true') {
            env.CI_TAGS = 'amd64-arch-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST + '|arm64v8-arch-' + env.EXT_RELEASE_CLEAN + '-pkg-' + env.PACKAGE_TAG + '-pr-' + env.PULL_REQUEST
          } else {
          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    docker manifest create ${MANIFESTIMAGE}:arch ${MANIFESTIMAGE}:amd64-arch ${MANIFESTIMAGE}:arm64v8-arch
                    ...
                    docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-arch || :
                    docker manifest create ${MANIFESTIMAGE}:arm32v7-arch ${MANIFESTIMAGE}:amd64-arch
                    docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-arch
                    docker manifest push --purge ${MANIFESTIMAGE}:arch
                    docker manifest push --purge ${MANIFESTIMAGE}:${META_TAG} 
                    docker manifest push --purge ${MANIFESTIMAGE}:${EXT_RELEASE_TAG} 
